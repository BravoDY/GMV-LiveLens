...
def test_dashboard_mobile_responsive_rules_hide_time_and_prevent_overflow() -> None:
    ...
    assert "20260518-total-complete-0005" in official_content
    assert "20260518-total-complete-0005" in test_content
...
def test_dashboard_summary_grid_uses_fluid_resize_observer() -> None:
    ...
    assert 'row.dataset.layout = "single-line";' in content
    assert 'row.dataset.layout = "stacked";' in content
    assert "const minSingleLineWidth = 188;" in content
    assert "const minTotalSingleLineWidth = 250;" in content
    assert "singleLineUnitWidth >=" in content
