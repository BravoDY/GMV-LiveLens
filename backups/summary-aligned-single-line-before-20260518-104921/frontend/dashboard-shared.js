...
function updateSummaryCardLayout() {
  ...
  const columns = Math.max(2, Math.min(maxColumns, rawColumns || 2));

  let totalSpan = 2;
  if (columns >= 6) totalSpan = 3;
  if (columns <= 3) totalSpan = 1;
  if (columns <= 2) totalSpan = 2;

  row.style.setProperty("--summary-columns", String(columns));
  row.style.setProperty("--total-span", String(Math.min(totalSpan, columns)));
}
