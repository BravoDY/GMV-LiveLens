...
def test_dashboard_mobile_responsive_rules_hide_time_and_prevent_overflow() -> None:
    ...
    assert "grid-template-columns: repeat(4, minmax(0, 1fr));" in styles_content
    assert "@media (min-width: 1560px)" in styles_content
    assert "grid-template-columns: repeat(6, minmax(0, 1fr));" in styles_content
    assert "grid-column: span 3;" in styles_content
    ...
    assert "20260518-platform-ratio-0002" in official_content
    assert "20260518-platform-ratio-0002" in test_content
