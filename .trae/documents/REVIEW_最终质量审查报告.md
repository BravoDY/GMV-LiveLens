...
| 检查项 | 方式 | 可信度 |
|--------|------|--------|
| full_test.py 语法 | py_compile 编译检查 | 高 |
| F3/F4 修复代码 | 代码 diff 检查 | 高 |
| .gitignore WAL 规则 | 文件内容读取 | 高 |
| .env 密码清空 | 文件内容读取 | 高 |
| data/.mysql_password 存在 | 文件系统检查 | 高 |
| main.py 版本号 | 文件内容读取 | 高 |
| 70/73 PASS 结果 | VERIFY 阶段 stdout 捕获 | 高 |
| 服务启动 HTTP 200 | urllib 请求验证 | 高 |
| 8 个 API 端点 | urllib 请求验证 | 高 |
| 8 个异常边界 | urllib 请求验证 | 高 |

### A.2 未验证（沙箱限制，需本地确认）
| 检查项 | 限制原因 | 可信度 | 本地验证命令 |
|--------|----------|--------|-------------|
| ruff check | sandbox 无 CLI 权限 | 低（仅语法检查） | ruff check backend/ |
| F3/F4 运行时 | sandbox PermissionError | 中（代码已修改） | python full_test.py --skip-api |
| Edge 真实恢复 | 无 Edge 浏览器 | 低 | 手动启动 Edge 后测试 |
| OCR 真实精度 | 无真实平台页面 | 低 | 采集真实页面验证 |
| MySQL 连接 | 主机不可达 | 低 | 本地启动后访问 /dashboard-test |
| WebSocket 实时推送 | 仅握手测试 | 中 | 启动采集后观察看板 |

### A.3 测试可靠性结论
- **直接验证可信**: 15/21 项（代码检查 + API 请求）
- **间接验证**: 3/21 项（代码已修改，语法通过，未运行时验证）
- **无法验证**: 3/21 项（依赖外部环境：Edge/MySQL/OCR 真实数据）
- **本地需补验**: 6 项（见上表本地验证命令）
...
### B.1 核心流程闭环
```
shops.csv(9家) -> init_db -> sync_tasks -> 9 tasks
                                              |
scheduler._run_loop -> capture_once -> Edge CDP 截图 -> OCR extract_candidates
                                              |                    |
                                           _judge()  <-------- candidate scores
                                              |
                                    store.add_sample -> WS broadcast -> 前端看板
```
关键检查：
- 9 家店铺全部有对应任务，无孤立数据 [PASS]
- 所有 shop_id 和 debug_port 唯一，无冲突 [PASS]
- task_to_dict() 输出包含前端所需全部核心字段 [PASS]

### B.2 数据口径一致性
- OCR 输出: extract_candidates 返回 [(amount, score), ...] 排序列表
- _judge 输入: 从候选列表选最高分 -> 跨天/下降/跳变/连续确认判定
- _judge 输出: (status, reason, trusted_value, pending_value, pending_count, required_confirms, accepted)
- store.add_sample: 保存 engine/candidates/confirms 到 gmv_samples 表
- WS broadcast: 通过 build_snapshot 序列化所有 task 状态
- 前端看板: 读取 last_trusted_value 显示当前 GMV
全链路数据类型一致（均为 int 金额），无单位混淆。[PASS]

### B.3 异常分支覆盖
- selected=None: parse_failed/needs_recalibration [PASS]
- 跨天下降: 接受新值（reset）[PASS]
- 同日下降: 忽略（退款/OCR截断）[PASS]
- 异常跳变: suspect + 提高确认次数 [PASS]
- 低置信来源: 额外确认次数 [PASS]
- last_success_at 异常: try/except 兜底 + 日志 [PASS]
...
- MYSQL_PASSWORD: 环境变量优先，密钥文件回退，最终 ""
[PASS]

### C.2 异常处理质量
- scheduler.py: 0 个裸 except:（全部指定 Exception 类型）[PASS]
- store.py: 0 个裸 except: [PASS]
- 所有采集异常通过 _record_failure 写入 task runtime status [PASS]
- 关键路径异常有 logger.warning/exception 日志 [PASS]

### C.3 路径兼容性
- 全部使用 pathlib.Path，无硬编码反斜杠 [PASS]
- DB_PATH / SCREENSHOT_DIR / DATA_DIR 均动态构建 [PASS]
- Windows 专用项目，未使用 Linux 特有路径 [PASS]

### C.4 空数据处理
- _judge(None): 返回 parse_failed + pending_count=1 [PASS]
- shops.csv 空行: _read_csv_text_with_fallbacks 跳过 [PASS]
- OCR 候选为空: selected=None 分支 [PASS]
- MySQL 连接失败: 返回空列表 + logger.warning [PASS]
...
### D.1 危险函数扫描
- eval(): 后端代码中 0 处使用 [PASS]
- exec(): 后端代码中 0 处使用 [PASS]
- subprocess shell=True: 后端代码中 0 处使用 [PASS]
- __import__ 动态导入: 仅 full_test.py 使用 importlib.import_module（测试工具）[PASS]

### D.2 硬编码敏感信息
- 无硬编码密码 [PASS]
- 无硬编码 API Key [PASS]
- 无硬编码数据库连接字符串 [PASS]
- GMV_API_TOKEN 从环境变量读取（默认值 change-me-before-public-deploy 为占位符）

### D.3 路径注入风险
- 所有文件路径通过 pathlib.Path 构建，非字符串拼接 [PASS]
- 无 os.path.join(request_param, ...) 模式 [PASS]
- 静态文件服务通过 FastAPI StaticFiles，自动防目录穿越 [PASS]
